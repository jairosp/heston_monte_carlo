...
HestonSimulator::HestonSimulator(const HestonParameters &params, unsigned int seed)
    : params_(params), rng_(seed), normal_(0.0, 1.0) {}
...
PricingResult HestonSimulator::price_european_call(size_t num_paths, size_t num_steps,
    ...
    if (disc_scheme == DiscretizationScheme::QuadraticExponential) {
        ...
        QEcoefs.K1 = (params_.kappa * params_.rho / params_.sigma - 0.5) * gamma1 * dt -
                     params_.rho / params_.sigma;
        QEcoefs.K2 = (params_.kappa * params_.rho / params_.sigma - 0.5) * gamma2 * dt +
                     params_.rho / params_.sigma;
    ...
    for (size_t path = 0; path < num_paths; path++) {
        ...
        for (size_t step = 0; step < num_steps; step++) {
            if (disc_scheme == DiscretizationScheme::EulerMaruyama)
                eulerStep_(X, v, dt, sqrt_dt);
            else if (disc_scheme == DiscretizationScheme::QuadraticExponential)
                qeStep_(X, v, QEcoefs, dt);
        }
...
void HestonSimulator::eulerStep_(double &X, double &v, double dt, double sqrt_dt) {
    ...
    v += params_.kappa * (params_.theta - v_truncated) * dt +
         sqrt_v * params_.sigma * sqrt_dt * normals.z2;
}
...
void HestonSimulator::qeStep_(double &X, double &v, QECoefficients QEcoefs, double dt) {
    ...
    double s2 = v_prev * sigma2 * exp_kdt * one_minus_exp / params_.kappa +
                params_.theta * sigma2 * one_minus_exp * one_minus_exp / (2.0 * params_.kappa);
    ...
    if (params_.kappa < 1e-8) {
        ...
        double term2 = (params_.theta * params_.sigma * params_.sigma / (2.0 * params_.kappa)) *
                       std::pow(1.0 - exp_k, 2);
        s2 = term1 + term2;
    }
    ...
    X += params_.r * dt + QEcoefs.K0 + QEcoefs.K1 * v_prev + QEcoefs.K2 * v +
         std::sqrt(variance_term) * Z_x;
}