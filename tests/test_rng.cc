...
TEST_F(ReproducibilityTest, SameSeedSameResult) {
    ...
    PricingResult results1 = engine1.price_european_call(
        num_paths, num_steps, DiscretizationScheme::QuadraticExponential);
    PricingResult results2 = engine2.price_european_call(
        num_paths, num_steps, DiscretizationScheme::QuadraticExponential);
...
TEST_F(ReproducibilityTest, StandardErrorDecreasesWithMorePaths) {
    ...
    auto result_small =
        engine1.price_european_call(10000, num_steps, DiscretizationScheme::QuadraticExponential);
    auto result_large =
        engine2.price_european_call(100000, num_steps, DiscretizationScheme::QuadraticExponential);
